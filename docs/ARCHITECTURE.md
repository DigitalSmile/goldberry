...
- **A binding keeps an address; the handle is a shared constant.** A binding class looks a symbol up once and keeps the `MemorySegment` it got, then calls it through the `Downcalls` constant naming its signature — one unbound `MethodHandle` per signature, 56 of them behind 134 bindings. This is not a style preference: a handle bound to an address cannot be a compile-time constant in a GraalVM native image, and a `MethodHandle` that is not a constant is an interpreted lambda form rather than a call, which cost a factor of 450 per crossing (ADR-0161).
