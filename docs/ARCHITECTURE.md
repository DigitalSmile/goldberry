...
**A fast and modern UI toolkit for Java.** Part of [Scarlet Macaw OS](https://scarletmacaw.org), usable standalone on Linux, Windows, and macOS from day 1.
...
- **Cross-platform from the first commit.** Linux (Wayland/X11), Windows, macOS, and the Scarlet Macaw compositor are peer backends behind one SPI.
...
├──────────────────┬──────────────────┬──────────────────┤
│ SDL3             │ Headless         │ Scarlet Macaw    │
│ the desktop      │ CI / golden-     │ compositor       │
│ backend          │ image tests      │ backend          │
└──────────────────┴──────────────────┴──────────────────┘
```
...
| libxkbcommon | Keyboard translation          | **Linux/Scarlet backends only.** An implementation detail behind the Backend SPI, not part of Goldberry's public input model. |
...
- **Arena discipline:** one shared `Arena` per window for long-lived objects (fonts, Yoga config, xkb keymap); a confined per-frame arena for scratch (HarfBuzz buffers, glyph arrays, damage lists) — dies at frame end, zero GC pressure. Long-lived wrappers expose `close()`; a `Cleaner` is the safety net, never the mechanism.
- Upcalls (Yoga measure functions) use **one shared stub** dispatching on the node's context pointer. Upcall cost is benchmarked in the M1 vertical slice.

### 3.2 Native build & packaging

- A CMake superbuild — driven from Gradle by the `:natives` module — statically links Blend2D + Yoga + HarfBuzz (+ SDL3; + libxkbcommon on Linux) into **one shared library `libgoldberry`** per platform. One artifact, one version matrix.
- Distribution follows the LWJGL pattern: classifier jars — `goldberry-natives-{linux,windows,macos}-{x64,aarch64}` — extracted or `System.load`-ed at startup.
- CI: GitHub Actions matrix, every artifact built on a native runner. Four runner types produce six artifacts — MSVC targets ARM64 from an x64 host, and Xcode on Apple Silicon targets x86_64. The Linux legs build inside a `manylinux_2_28` container, which pins the glibc floor at 2.28 (RHEL 8) — a stock runner would link against glibc 2.39 and refuse to load on older distributions.
...
- **Backends shipped:** `sdl3` (the desktop backend on all three OSes), `headless` (renders to `BLImage` for golden-image tests), `scarlet` (OS compositor: shm buffers + later dmabuf). That is the complete list — no hand-written Win32/Cocoa/Wayland backends and no AWT bridge, ever. SDL3 is a permanent dependency on desktop; the SPI exists to serve `headless` and `scarlet`, not as an invitation to grow new desktop backends.
...
- **Layers & damage.** Render objects marked as repaint boundaries (scroll containers, animating nodes, the frost material, the 3D canvas) cache their raster as `BLImage`. A frame re-rasterizes only dirty layers and blits the rest; damage rects flow to the backend (the Scarlet compositor consumes them natively).
...
- **Keyboard:** `KeyEvent{ key, physicalCode, modifiers, repeat }` — separated from **text input**: `TextEvent{ committedText }`. On Linux/Scarlet the translation is libxkbcommon (`xkb_state` + `xkb_compose` for dead keys); on Windows/macOS the platform's own translation is used. This split keeps the door open for IME preedit later.
...
Contrast for text-on-semantic-color pairs is validated in CI against WCAG AA. Scarlet Macaw OS maps its `--os-*` variables onto `--gb-*` in its theme layer; standalone apps never see `--os-*`.
...
**Menus & shell (`goldberry-widgets`):** `menubar`, `menu`, `item`, `context-menu` (any widget: `context-menu=` reference), all rendered in backend popup windows (`SDL_CreatePopupWindow` / Scarlet popup surfaces) so they can escape the window bounds; `tray-icon` (SDL3 ≥ 3.2 tray API; Scarlet backend maps to the OS shell) with menu + tooltip; **window decorations** — native by default, optional client-side decorated `titlebar` widget (frost material, window buttons per-platform ordering).
...
- `BackendWindow.gpuSurface()` is present in the SPI from day 1; day-1 backends may return empty without breaking anything. The Scarlet compositor gains dmabuf buffer acceptance for mode 2 (tracked in the OS design doc).
...
- License: toolkit Apache-2.0. Bundled assets — Inter (OFL), JetBrains Mono (OFL), Lucide (ISC), OpenMoji derivative (CC BY-SA, attribution in About/NOTICE). The statically linked native libraries also redistribute in object form: Blend2D, AsmJit, SDL3 (Zlib), Yoga, HarfBuzz, libxkbcommon (MIT). Full disclosure in `THIRD-PARTY-NOTICES.md` and `licenses/`, verified by `./gradlew checkLicenses`; see ADR-0015.
...
- **M1 — Vertical slice:** styled wrapped paragraph, resized at 60 fps on Mac + ThinkPad + Scarlet compositor; paragraph cache + upcall benchmarks green.
...
- **M4 — GPU:** `canvas3d`, GPU composition path, dmabuf on Scarlet.
...
- **Open:** KDL `bind` expression scope (dotted paths only vs. mini-expressions); whether `--gb-selection` alpha compositing forces a color-mix() subset; CSD-by-default on Scarlet vs. everywhere.