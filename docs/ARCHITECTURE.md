...
**Primitives (`goldberry-core`):** `text`, `icon`, `image` (Blend2D codecs), `spacer`, `row`, `column`, `stack` (z-layering), `scroll` (design-system scrollbar spec: overlay, hover-widening), **`canvas` is built** (immediate-mode Blend2D painting surface — the escape hatch and the substrate for charts): a `Painter` is a content slot on `Box`, handed the frame translated to the box's content corner and clipped to it, inside the `save`/`restore` pair the export list grew for it (ADR-0193), `focus-scope`.
...
| `goldberry-web` | — | Servo (MPL-2.0) | **parked**: libservo is Rust-only against an unstable API |
