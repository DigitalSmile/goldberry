...
- Emoji font: **Noto Color Emoji**, the **COLRv1 build** (`Noto-COLRv1.ttf`, 5 MB, SIL OFL 1.1), unmodified ([ADR-0456](../book/src/adr/0456-the-emoji-face-is-noto-drawn-from-its-paint-graphs.md)). It replaced OpenMoji's COLRv0 build. The palette is Noto's own and is not re-themed. No bitmap emoji formats (CBDT/sbix): Noto's `CBDT` build is 10.7 MB of 136 px strikes that blur above the size they were drawn at.
- COLRv1 is a **paint graph** per glyph: outlines that clip solids and linear, radial and sweep gradients, under affine transforms, with composites blending subtrees. `text.font.sfnt.ColorPaints` reads it in Java into `ColorPaint`, a sealed tree of records, parsing each glyph's graph on first use; `GlyphOutlines` reads `glyf`, because a glyph in the graph is a *clip* and the rasterizer's glyph call only fills with a colour. `paint.ColourGlyphPainter` draws the tree: a clipped brush is a `fillPath` of the outline, a transform between an outline and its gradient becomes the gradient's own matrix, and a composite renders both sides into offscreen `Layer`s the size of the glyph's clip box on the device and blits one onto the other with the font's operator. Version 0 (`ColorLayers`) is still read, for any face that is still version 0; where a face carries both, the graph wins.
...
- **The face is not in `goldberry-core`**: it ships as `goldberry-emoji`, an optional artifact an application adds on purpose, because five megabytes of emoji should not be inherited by an application that never draws one. It moved there when the face was OpenMoji, whose CC BY-SA wanted credit on screen; Noto's OFL does not. `:core` names the slot and loads the face through an `EmojiFont` service; with no provider, `Font.bundled(EMOJI, …)` fails with a sentence naming the artifact ([ADR-0384](../book/src/adr/0384-the-emoji-face-is-an-artifact-an-application-opts-into.md)). A stylesheet reaches it as `font-family: "Noto Color Emoji"`.
...
- License: toolkit Apache-2.0. Bundled assets — Inter (OFL), JetBrains Mono (OFL), Lucide (ISC) in `goldberry-core`; Noto Color Emoji (OFL) in the optional `goldberry-emoji` (ADR-0384, ADR-0456). The statically linked native libraries also redistribute in object form: Blend2D, AsmJit, SDL3 (Zlib), Yoga, HarfBuzz. On Linux SDL loads the system libxkbcommon at run time; it is not redistributed. Full disclosure in `THIRD-PARTY-NOTICES.md` and `licenses/`, verified by `./gradlew checkLicenses`; see ADR-0015.
