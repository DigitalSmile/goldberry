...
- **Property split** (already a design invariant):
    ...
    - Of that list, `ComputedStyle` resolves **`background`, `color`, `opacity`, `border-radius` / `border` / `outline` (one radius, not per-side), `font-family` / `font-size` / `font-weight` / `line-height`, `cursor` and `transition`**. `box-shadow`, `backdrop-filter`, `letter-spacing`, `text-align` and `transform` are absent, because `Box` cannot express them and a property that resolves into nothing is a property with no test that means anything. Each arrives with the thing that paints it.
...
**Two design-system sections have no counterpart here and are not duplicated on purpose:** §1.3's 4 px spacing ramp and `--gb-density`, and §1.5's radii, elevation levels and the three materials. `--gb-density` is **not implemented** — there is one density and control heights are the `regular` column. The radii are implemented as values in `controls.css`; elevation and the `frost`/`veil` materials are not (§5 describes the blur path that would serve them).
...
- **M2 — Widgets & style:** CSS engine, KDL inflater + hot reload, core controls, Nord light/dark, focus/cursor/shortcuts, golden-image CI. *Engines done; two of thirteen controls built. `book/src/status.md` has the detail.*
