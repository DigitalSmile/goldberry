...
- **`segmented`** — a row of mutually exclusive options rendered as one joined control, sharing `radio-group`'s model and invariant exactly: one Tab stop, arrows rove, exactly one selected, `change` reports the value. It is `radio-group` with a different drawing, and it is a separate widget rather than `radio-group.segmented` because the two are not substitutable in a layout — a segmented control is a fixed-width bar that belongs in a toolbar, a radio group is a list that belongs in a form. Options are inline `option` children with a label, an icon, or both; icon-only segments require `name=`. Semantics: radiogroup/radio.
