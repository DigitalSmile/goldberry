...
- **The catalogue sweeps read the registry, not a list.** `WidgetParityTest`, `ImmutabilityTest` and `ChainingTest` walk `Widgets.inflater().registered()` — all 72 widgets — through a test-scope `CatalogMarkup` that supplies, per type, the *arguments* the §13 widgets refuse to exist without. They enumerated 24 names until the 2026-09-18 review counted them, so everything under `form`, `panel`, `menu`, `nav`, `overlay` and `data` was outside all three. `WidgetParityTest` is 203 tests now rather than 40. What the widening found is written in `review-2026-09-18-fixes.md`; the two widgets it has to exempt are exempt **by name, with the reason**, and each sweep has a `theExemptionsAreLive` test so an exemption cannot go stale or start passing unnoticed.
...
- **Blocking after triage:** PMD, with a hand-picked ruleset in `config/pmd/ruleset.xml` — PMD's defaults are a style guide, and this codebase's comments are prose and its painters are long because a rasterizer step is long. SpotBugs is not wired: two overlapping bytecode analysers is a second report to triage for the same findings.
  ...
  **`module-info.java` is excluded from PMD on purpose.** PMD's Java grammar takes at most one modifier on a `requires` directive, and every module here declares `requires transitive static org.jspecify` — both, which the JLS has allowed in either order since Java 9. PMD 7.19 and 7.20 both stop at the second one, so every module descriptor was reported as a `ParseException` and analysed by nothing. A descriptor has no resource to leak and no string built in a loop, so the ruleset has nothing to say about it; the exclusion says that where a stack trace in every report did not ([ADR-0398](../book/src/adr/0398-the-build-declares-what-it-actually-writes.md)).
- **The published javadoc is a gate, not a release step.** `check` depends on `javadoc` in every module `goldberry.publish` applies to, with doclint on less the `missing` group ([ADR-0343](../book/src/adr/0343-the-published-javadoc-is-linted.md)), so a rotted `[link]` fails the Linux, macOS and Windows Java jobs about four minutes in. It used to fail nowhere until `publish.yml`'s last step — thirteen minutes in, and partway through an upload that had already put three modules on Central ([ADR-0405](../book/src/adr/0405-check-generates-the-published-javadoc.md)). `:assets` and `:weaver` are build-time modules, are not published, and are not linted; both have `reference not found` errors that a decision to publish either would have to clear.
