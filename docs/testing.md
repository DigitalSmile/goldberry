...
| showcase | `showcase.yml` | `v*` tag, or by hand | The GraalVM native image on three platforms, each run until it presents three frames; on a tag, attached to the draft GitHub Release (ADR-0337, ADR-0340). The example's own tests against a built library run on `linux.yml`'s linux-x64 verify leg on every push |
