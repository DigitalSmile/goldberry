...
| `link` | `body`; underline on hover and always in `:focus-visible`; external icon 12 with gap 4. **Not built**, and not the same thing as `button.link`: this is §2's *text* widget — a word inside a sentence, where the underline is WCAG's "never by colour alone" and therefore not optional. It needs `text-decoration` in §8's subset, which is why it is still waiting |
