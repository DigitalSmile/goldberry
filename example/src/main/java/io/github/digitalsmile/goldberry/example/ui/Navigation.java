...
public record Navigation(ShowcaseModel model, ShowcaseModel.Actions actions, Runnable startTour)
    ...
    public Widget build(BuildContext context) {
        return new Wall("navigation", "Navigation", NOTE, 2, List.of(
    ...
    private Widget tour() {
        return Notifications.card("tour-card", "A guided walk", List.of(
                new Text("A tour is a veil with a hole in it, a ring around the thing being"
                ...
                new Button("Take the tour", startTour)
                        .withAttributes(Attributes.NONE.id("tour-button").classes("primary")),
                new Text("Escape ends it; Enter and the arrow keys move between stops. The"
                        + " three stops are on this screen, so nothing has to switch tabs"
                        + " underneath you.", Attributes.NONE.classes("caption"))));
    }
}
