...
- **`--gb-density` is not implemented.** §1.3 specifies `regular | compact` with
  control heights 32/28 and list rows 32/26, "a user preference applied app-wide;
  token-conformant apps adapt with zero code". Every control's height is a
  literal `32px` in `controls.css` rather than `var(--gb-control-height)`, so
  there is nothing for a density to swap. It is a token-plumbing change across
  every control at once rather than anything per widget, which is why it is
  recorded here rather than counted against `radio`. —
