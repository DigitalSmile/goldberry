...
public final class BindingRegistry {
    ...
    private final Map<String, Observable<?>> byPath = new LinkedHashMap<>();
    private final boolean strict;

    private BindingRegistry(boolean strict) {
        this.strict = strict;
    }
    ...
    /// that never changes.
    public static BindingRegistry strict() {
        return new BindingRegistry(true);
    }
    ...
    public static BindingRegistry lenient() {
        return new BindingRegistry(false);
    }

    /// No bindings at all, and no complaints.
    public static BindingRegistry none() {
        return lenient();
    }

    /// Binds a path to a value.
    ///
    /// An [Observable] and not a [Property], because the two things an
    /// application publishes are the same thing seen from two sides: a
    /// `Property` it declared, or a raw field the weaver rewired into one
    /// ([Model]). The registry hands out the read-only half either way, so it
    /// has no reason to know which it was given.
    ///
    ...
    ///         changing by itself
    public BindingRegistry bind(String path, Observable<?> property) {
        requirePath(path);
    ...
    public BindingRegistry rebind(String path, Observable<?> property) {
        requirePath(path);
    ...
    public Map<String, Observable<?>> bound() {
        ...
        // this is what the "Bound: ..." half of a strict registry's error message
        // prints, and a set of paths in hash order is a worse thing to read than
        // the order they were declared in.
        return Collections.unmodifiableMap(new LinkedHashMap<>(byPath));
    }
