...
public final class Goldberry {
    ...
    /// What this build of the toolkit can actually do, whatever its API says.
    ///
    /// The platform integrations — the desktop's theme, its file dialogs, an input
    /// method, device hotplug, screensaver inhibition — are compiled into the
    ...
    ///     // Follow the user's own choice, and say why rather than blaming the desktop.
    /// }
    /// ```
    ///
    /// Answered from the loaded library's own record of how it was built, so it
    /// may be asked before a window is open. Empty where there is no native
    /// library at all.
    ///
    ...
    public static boolean isUiThread() {
        return GoldberryRuntime.isStarted()
                && GoldberryRuntime.get().loop().ui().isUiThread();
    }
    ...
    private static String loadVersion() {
        try (InputStream in = Goldberry.class.getResourceAsStream(BUILD_INFO_RESOURCE)) {
            if (in == null) {
                throw new IllegalStateException(BUILD_INFO_RESOURCE + " is missing from the goldberry-core artifact");
            }
            var properties = new Properties();
            properties.load(in);
            var version = properties.getProperty("version");
            if (version == null || version.isBlank()) {
                throw new IllegalStateException(BUILD_INFO_RESOURCE + " does not declare a version");
            }
            return version;
        } catch (IOException e) {
